...
- **SnapshotReminderService** -- `@Observable @MainActor class` singleton (NSObject + `UNUserNotificationCenterDelegate`), reconciliation manager for opt-in periodic snapshot reminders. Three mutators (`setEnabled`, `updateConfig`, `recordSnooze`) update `SettingsService` and enqueue `reconcile()` onto a serial Task chain; reconcile wipes the `snapshotReminder.*` namespace and rebuilds from desired state. `reconcileOnLaunch` self-heals drift. Notification body contains no financial data. See `Documentation/BusinessLogic.md` for cadence rules and `docs/superpowers/specs/2026-05-07-snapshot-reminder-reconciliation-design.md` for the design.
