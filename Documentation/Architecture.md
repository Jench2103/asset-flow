...
1. **SnapshotReminderService** (`@Observable @MainActor` `NSObject` subclass with `static let shared` singleton): Single owner of opt-in periodic snapshot reminders, implemented as a **reconciliation manager**. Three public mutators (`setEnabled`, `updateConfig`, `recordSnooze`) update `SettingsService` and enqueue a private `reconcile()` onto a strict serial Task chain; reconcile wipes the entire `snapshotReminder.*` namespace in `usernoted` and rebuilds from desired state (recurring requests for the cadence + one snooze per future `Date` in `settings.activeSnoozes`). `reconcileOnLaunch` runs at every app launch and self-heals any drift. Conforms to `UNUserNotificationCenterDelegate` (via `nonisolated` extension methods) to surface foreground banners and route the **Remind Tomorrow** action through `recordSnooze`. Tests inject a fake `UNUserNotificationCenterProtocol` and an isolated `SettingsService`; design rationale lives in `docs/superpowers/specs/2026-05-07-snapshot-reminder-reconciliation-design.md`.
