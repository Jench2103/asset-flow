...
1. **Notifications**:
   - **Snapshot Reminders** toggle: Enables/disables periodic local notifications that nudge the user to capture a new portfolio snapshot. Off by default. Toggling on requests the macOS notification authorization prompt at that moment. Failure to authorize surfaces one of two alerts depending on history (see `BusinessLogic.md` → Failure modes): a **Notifications Disabled** alert with an **Open System Settings** deep link when the user previously authorized and has since disabled it, or a **Couldn't Enable Notifications** alert with a **Report on GitHub** action when the bundle was never registered with the notification daemon (e.g. the app is running from a transient location).
